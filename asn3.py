...
def gainWallFollow(gain):
	...
	while k < duration:
		...
		error = gain*(target - rightSensor)
		setMotorTargetPositionCommand(1, 212+error)
		setMotorTargetPositionCommand(2, 812+error)
		total_error += error
		pause(.5)
...
if __name__ == "__main__":
	...
	#-----GAIN TUNING--------
		...
		while tolerance > .01: #I have no idea what this value should be
			...
			error1 = gainWallFollow(gain1):
			raw_input("Press enter to start test")
			error2 = gainWallFollow(gain2):
			raw_input("Press enter to start test")
			error3 = gainWallFollow(gain3):
